...
const obtenerCursoPublicado = async (req, res) => ok(res, await academicoService.getCursoEstructura(req.params.id, { publicado: true, usuarioId: req.user.id }));
...
const completarLeccion = async (req, res) => ok(res, await academicoService.completeLeccion(req.user.id, req.params.id, req.body.completado !== false));
